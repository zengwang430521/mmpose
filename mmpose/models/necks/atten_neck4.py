...
class MergeAttention(nn.Module):
    ...
    def forward(self, tar_dict, gather_dict, src_dict=None):
        ...
        map_size = gather_dict['map_size']
        x_source, _ = token2map_agg_mat(
            src_dict['x'],
            None,
            loc_orig=src_dict['loc_orig'],
            idx_agg=src_dict['idx_agg'],
            map_size=map_size)
        x_source = x_source.flatten(2).permute(0, 2, 1)
