...
class MyPVT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(2):
            ...
            outs.append((x, loc, [H, W], loc, idx_agg))
        ...
        for i in range(2, self.num_stages):
            ...
            block = getattr(self, f"block{i + 1}")
            norm = getattr(self, f"norm{i + 1}")
            x, loc, idx_agg, agg_weight = down_layers(x, loc, loc_orig, idx_agg, agg_weight, H, W, N_grid)  # down sample
            H, W = H // 2, W // 2

            for j, blk in enumerate(block):
                x = blk(x, loc, idx_agg, agg_weight, loc_orig, x, loc, idx_agg, agg_weight, H, W, conf_source=None)
