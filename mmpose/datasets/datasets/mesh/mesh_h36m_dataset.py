...
@DATASETS.register_module()
class Mesh3DPWDataset(MeshBaseDataset):
    """Human3.6M Dataset for 3D human mesh estimation. It inherits all function
    from MeshBaseDataset and has its own evaluate fuction.
    ...
    def evaluate(self, outputs, res_folder, metric='joint_error', logger=None, **kwargs):
        """Evaluate 3D keypoint results."""
        metrics = metric if isinstance(metric, list) else [metric]
        allowed_metrics = ['joint_error']
        for metric in metrics:
        ...
        for out in outputs:
            for (keypoints, image_path, keypoints_gt, error_vertices) in \
                    zip(out['keypoints_3d'], out['image_path'], out['keypoints_3d_gt_mesh'], out['error_vertices']):
                kpts.append({
                    'keypoints': keypoints.tolist(),
                    'image': image_path,
    ...
    def _report_metric(self, res_file):
        ...
        pred_joints_3d = [pred['keypoints'] for pred in preds]
        # gt_joints_3d = [item['joints_3d'] for item in self.db]
        gt_joints_3d = [pred['keypoints_gt'] for pred in preds]
        ...
        gt_joints_visible = np.ones([pred_joints_3d.shape[0], pred_joints_3d.shape[1]]) > 0
        mpjpe = keypoint_mpjpe(pred_joints_3d, gt_joints_3d, gt_joints_visible)
        mpjpe_pa = keypoint_mpjpe(
            pred_joints_3d,
            gt_joints_3d,
